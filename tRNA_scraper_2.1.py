...
def find_sequence_end_index(secondary_structure, loop_type, interval): #interval must be 1 or -1
    stem_counter = 0
    stem_direction = 0  # 1 is >, -1 is <
    direction_turn_count = 0
    last_same_direction_index = 0

    acceptor_5end_base_count = 0
    acceptor_5end_start_index = 0
    while not secondary_structure[acceptor_5end_start_index] == '>':
        acceptor_5end_start_index += 1

    acceptor_5end_end_index = acceptor_5end_start_index
    while secondary_structure[acceptor_5end_end_index] == '>':
        acceptor_5end_end_index += 1
        acceptor_5end_base_count += 1
    ...
    if interval == 1:
        ...
        index = acceptor_5end_end_index
        while stem_counter < stem_counter_requirement:
            new_stem_direction = 0
            if secondary_structure[index] == '>':
                new_stem_direction = 1
            elif secondary_structure[index] == '<':
                new_stem_direction = -1

            if new_stem_direction != 0:
                if stem_direction == -1 * new_stem_direction:
                    direction_turn_count += 1
                stem_direction = new_stem_direction
                if direction_turn_count % 2 == 1:
                    last_same_direction_index = index
            stem_counter = direction_turn_count // 2
            index += 1

        if loop_type == 'vLoop':  # fixme make it more elegant later
            last_same_direction_index += 1
        elif loop_type == 'acLoop':
            while secondary_structure[last_same_direction_index] != '>':
                last_same_direction_index += 1
        ...
        acceptor_3end_base_count = 0
        acceptor_3end_start_index = len(secondary_structure) - 1
        while not secondary_structure[acceptor_3end_start_index] == '<':
            acceptor_3end_start_index -= 1

        acceptor_3end_end_index = acceptor_3end_start_index
        while acceptor_3end_base_count < acceptor_5end_base_count:
            if secondary_structure[acceptor_3end_start_index] == '<':
                acceptor_3end_base_count += 1
            acceptor_3end_end_index -= 1
        index = acceptor_3end_end_index

        if loop_type == 'tLoop':  # fixme
            while secondary_structure[index] == '<':
                index -= 1
            return index
    ...
    return last_same_direction_index
...
def select_tLoop(): # fixme not giving the right results
    global chosen_sequence
    global chosen_secondary_structure
